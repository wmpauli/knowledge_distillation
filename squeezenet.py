...
def SqueezeNet(weight_decay, image_size=224, trainable=False, weight_file='squeezenet_weights.hdf5'):
    ...
    x = fire_module('2', x, squeeze=16, expand=64, trainable=trainable) # 55, 55, 128
    x = fire_module('3', x, squeeze=16, expand=64, trainable=trainable) # 55, 55, 128
    x = MaxPooling2D(pool_size=(3, 3), strides=(2, 2))(x) # 27, 27, 128

    x = fire_module('4', x, squeeze=32, expand=128, trainable=trainable) # 27, 27, 256
    x = fire_module('5', x, squeeze=32, expand=128, trainable=trainable) # 27, 27, 256
    x = MaxPooling2D(pool_size=(3, 3), strides=(2, 2))(x) # 13, 13, 256
    
    x = fire_module('6', x, squeeze=48, expand=192, trainable=trainable) # 13, 13, 384
    x = fire_module('7', x, squeeze=48, expand=192, trainable=trainable) # 13, 13, 384
    x = fire_module('8', x, squeeze=64, expand=256, trainable=trainable) # 13, 13, 512
    x = fire_module('9', x, squeeze=64, expand=256, trainable=trainable) # 13, 13, 512
